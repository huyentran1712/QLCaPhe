...
namespace QuanLyCaPhe
{
    public partial class KHUVUC : Form
    {
        ...
        {
            string sqlClear = "delete from KHUVUC where @MaKhuVuc = KHUVUC";
        ...
        {
            string sqlupdate = "update KHUVUC set TENKHUVUC = @TenKhuVuc where KHUVUC = @MaKhuVuc";
        ...
        {
            string strFind = "select *, '' from KHUVUC where KHUVUC = @MaKhuVuc";
            SqlCommand command = new SqlCommand(strFind, connection);

            command.Parameters.AddWithValue("KHUVUC", tb_find_KV.Text);
